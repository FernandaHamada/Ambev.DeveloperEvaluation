...
public class UpdateUserHandler : IRequestHandler<UpdateUserCommand, UserResult>
{
    ...
    public async Task<UserResult> Handle(UpdateUserCommand command, CancellationToken cancellationToken)
    {
        ...
        var userMap = _mapper.Map<User>(command);

        var updatedUser = await _userRepository.UpdateAsync(userMap, cancellationToken);

        if (updatedUser == null)
            throw new KeyNotFoundException($"User with ID {command.Id} not found");

        var result = _mapper.Map<UserResult>(updatedUser);
